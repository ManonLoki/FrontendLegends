...
Godot 4.7 在 macOS 受限环境中无法写入默认 `user://logs` 时可能崩溃。仓库内的命令行启动、测试和自动化必须经 `tools/godot-safe.sh` 显式写入 `/tmp` 日志；完整规范见 `docs/godot_stability.md`。
...
9. **Godot 稳定入口：** 禁止自动化直接调用 `Godot` 二进制。编辑器、游戏、无界面测试和导出必须通过 `tools/godot-safe.sh`，或像 `tools/build-web.mjs` 一样显式传入位于可写临时目录的 `--log-file`。若出现 `Failed to open 'user://logs/...'` 后接 `signal 11`，应视为启动入口违规并改用安全脚本，不得误判为游戏逻辑崩溃，也不得通过删除项目数据或正式存档处理。
