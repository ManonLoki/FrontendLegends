...
9. **Godot 稳定入口：** 禁止自动化直接调用 `Godot` 二进制。编辑器、游戏、无界面测试和导出必须通过 `tools/godot-safe.sh`，或显式传入位于可写临时目录的 `--log-file`。若出现 `Failed to open 'user://logs/...'` 后接 `signal 11`，应视为启动入口违规并改用安全脚本，不得误判为游戏逻辑崩溃，也不得通过删除项目数据或正式存档处理。
