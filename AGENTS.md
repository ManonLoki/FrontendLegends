...
命令行启动、测试和自动化必须使用 `tools/godot-safe.sh`（详见规则 9，完整规范见 `docs/godot_stability.md`）。
...
8. **冻结的基础玩法不变量：** `docs/design_invariants.md` 中的学习与战斗基础逻辑未经用户单独且明确的二次确认不得修改。学习必须保持“潜能按灵感转换为固定口径学习经验 → 经验满 → 按实际潜能消耗支付 Token → 升级”；固定升级经验不得读取灵感。战斗必须保留四属性、已装备功法、装备和显式状态/阶位/AI 修正的共同作用及规定的核心结算阶段。允许在不改变资源身份、输入维度、职责和阶段顺序的前提下调整数值、系数、阈值、概率、曲线和计算公式。首次提出破坏不变量的修改不算确认；编辑前必须停止，列出准确不变量、文件、行为、数据/存档影响与拟更新测试，再请求后续一次性解锁。规范副本见 `CLAUDE.md`。
9. **Godot 稳定入口：** 禁止自动化直接调用 `Godot` 二进制。编辑器、游戏、无界面测试和导出必须通过 `tools/godot-safe.sh`（`tools/build-web.mjs` 的 Web 导出亦经由它执行），或显式传入位于可写临时目录的 `--log-file`。若出现 `Failed to open 'user://logs/...'` 后接 `signal 11`，应视为启动入口违规并改用安全脚本，不得误判为游戏逻辑崩溃，也不得通过删除项目数据或正式存档处理。
