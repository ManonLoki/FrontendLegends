...
func _run() -> void:
	...
	state.delete_save()
	_assert_true(state.current_save_path().begins_with("user://test_saves/"), "数值测试必须使用隔离的 user://test_saves/ 存档")
...
func _test_small_student_ttk(state: Node, npc_system: Node, combat: Node) -> void:
	...
	var attacks := _simulate_player_attacks(combat, "xiao_xue_sheng", TRIALS, 20260716)
	...
	var balanced_student := _simulate_player_attacks(combat, "xiao_xue_sheng", TRIALS, 20260717)
	...
	var qc_attacks := _simulate_player_attacks(combat, "qc", TRIALS, 20260718)
	var qc_median := _percentile(qc_attacks, 0.50)
	var qc_p90 := _percentile(qc_attacks, 0.90)
	_assert_true(qc_median >= 6 and qc_median <= 8, "均衡新手击败 Qc. 的攻击次数中位数应为 6–8，当前为 %d" % qc_median)
	_assert_true(qc_p90 <= 10, "均衡新手击败 Qc. 的攻击次数 P90 应不超过 10，当前为 %d" % qc_p90)
...
func _test_qc_reward(registry: Node) -> void:
	var qc: Dictionary = registry.get_npc("qc")
	var reward: Dictionary = qc.get("combatReward", {})
	_assert_true(str(qc.get("combatRank", "")) == "novice" and str(qc.get("combatRole", "")) == "balanced", "Qc 应是 novice/balanced 训练目标")
...
func _test_combat_reward_scaling(resolver: Node) -> void:
	...
	for rank in expected:
		var reward: Dictionary = resolver._combat_reward({"combatRank": rank, "skillLevels": {"basicStrength": 20}})
	...
	var configured := {"combatRank": "legendary", "skillLevels": {"basicStrength": 200}, "combatReward": {"experience": 7, "potential": 3, "money": 2}}
...
func _test_victory_reward_exclusion(state: Node, registry: Node, npc_system: Node, quests: Node, resolver: Node) -> void:
	...
	npc_system.register_runtime(ENEMY_ID, {"displayName": "任务奖励互斥目标", "combatRank": "legendary", "combatRole": "balanced", "attributes": {"strength": 20, "agility": 20, "constitution": 20, "wisdom": 20}, "skillLevels": {"basicStrength": 100}, "combatReward": {"experience": 999, "potential": 999, "money": 999}})
...
func _test_noncombatant_quest_filter(registry: Node, quests: Node) -> void:
	...
	var combat_filter_fixture: Array[Dictionary] = [
		{"npc_id": "xiao_xue_sheng", "map_id": "test", "map_name": "测试"},
		{"npc_id": "nai_cha_mei_mei", "map_id": "test", "map_name": "测试"},
		{"npc_id": "qc", "map_id": "test", "map_name": "测试"},
	]
	registry.placed_npc_targets = combat_filter_fixture
	seed(20260718)
	var selected: Dictionary = quests._placed_npc_target([], true)
	_assert_true(str(selected.get("npc_id", "")) == "qc", "combat_only 目标池必须排除全部 noncombatant")
...
func _test_endgame_weapon_roles(state: Node, registry: Node) -> void:
	var bandage_effects: Dictionary = registry.get_item("chuang_ke_tie").get("effects", {})
	var antibiotic_effects: Dictionary = registry.get_item("amoxicillin").get("effects", {})
	...
	var mbp_bonus: Dictionary = registry.get_item("mbp").get("equipmentBonus", {})
	var gpu_bonus: Dictionary = registry.get_item("gpu_5090d").get("equipmentBonus", {})
