...
func _run_hud_suite() -> Node:
	...
	_assert_true(web_builder.contains("packManifest") and web_hasher.contains("__CONTENT_PACK_MANIFEST__"), "Web 构建必须把内容寻址的分包清单注入页面")
	...
	_assert_true(export_presets.count("platform=\"Web\"") == 1 and export_presets.contains("\"Web Split\"=\"Web Split\""), "项目必须只保留 Web Split 导出预设")
	_assert_true(export_presets.count("include_filter=\"*.ogg,*.tmx,*.tsx,*.tpsheet\"") == 3, "macOS、Windows 与 Android 导出必须包含音频、运行时地图和图集定义")
	_assert_true(export_presets.contains("name=\"Web Split\"") and export_presets.contains("custom_features=\"split_content_packs\"") and export_presets.contains("export_filter=\"exclude\""), "正式 Web 构建必须使用内容分包预设")
	_assert_true(export_presets.count("include_filter=\"*.tmx,*.tpsheet\"") == 1, "Web Split 主包只应保留运行时 TMX 与图集定义")
	_assert_true(export_presets.contains("assets/Map/assets/*.png,assets/Map/tilesets/*.tsx") and export_presets.contains("dist/*,node_modules/*") and export_presets.contains("tests/*,tools/*,docs/*"), "Web Split 主包必须排除分包资源与非运行文件")
	...
	for pack_definition in bgm_catalog.get("packs", {}).values():
		_assert_true(export_presets.contains(str(pack_definition.get("audio", ""))), "Web Split 必须显式排除每个地区音频资源")
