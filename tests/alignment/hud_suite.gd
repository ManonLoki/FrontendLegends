...
func _run_hud_suite() -> Node:
	...
	_assert_true(content_pack_source.contains("OS.has_feature(SPLIT_CONTENT_FEATURE)"), "内容包管理器只能在显式分包构建中下载外部资源")
	var export_presets := FileAccess.get_file_as_string("res://export_presets.cfg")
	_assert_true(export_presets.count("platform=\"Web\"") == 1 and export_presets.contains("Web=\"Web\""), "项目必须只保留一个 Web 导出预设")
	_assert_true(export_presets.count("include_filter=\"*.ogg,*.tmx,*.tsx,*.tpsheet\"") == 5, "所有平台导出都必须包含音频、运行时地图和图集定义")
	_assert_true(export_presets.contains("name=\"Web\"") and not export_presets.contains("custom_features=\"split_content_packs\"") and export_presets.contains("export_filter=\"all_resources\""), "正式 Web 构建必须由 Godot 面板直接导出完整资源包")
	_assert_true(not export_presets.contains("assets/Map/assets/*.png,assets/Map/tilesets/*.tsx") and export_presets.contains("dist/*,node_modules/*") and export_presets.contains("tests/*,tools/*,docs/*"), "Web 完整包不得排除运行时地图资源，但仍应排除开发文件")
	...
	_assert_true(map_transition_source.contains("ensure_map_pack(target_map_id)"), "地图切换必须兼容可选的外部分包模式")
