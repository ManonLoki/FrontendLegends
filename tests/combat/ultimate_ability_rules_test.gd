...
func _run() -> void:
	...
	_assert_true(int(tier_one.effect_level) == 74 and int(tier_two.effect_level) == 80, "80级后绝招一档效果应封在74级，二档使用完整功力，避免同效果不同价格")
	...
	var tier_one_label := RULES.format_ult_label(RULES.build_ult({"key": "combo", "names": ["一档", "二档"], "abilitySets": [["multi"], ["multi"]], "mpCosts": [25, 45]}, 1, 200, 100))
	_assert_true(tier_one_label.contains("4击") and tier_one_label.contains("每击60%"), "满级一档绝招仍应显示封顶后的低耗效果")
