...
func _run() -> void:
	...
	_assert(not handler.is_interactable({"properties": {"questGiver": "darkxue_computer"}}), "旧 questGiver 属性不得再被识别为世界事件")
	...
	var darkxue: TiledMapLoader = contexts["DarkXue"]
	var terminal: Dictionary = darkxue.interactable_object_at_tile(12, 6)
	_assert(str(terminal.get("properties", {}).get("action", "")) == "quest_endpoint", "DARK学电脑应由数据表注入任务终端行为")
	_assert(str(terminal.get("properties", {}).get("worldEventId", "")) == "darkxue_computer", "注入对象应保留稳定事件 ID")

	var kaiyuan: TiledMapLoader = contexts["KaiyuanTown"]
	_assert(str(kaiyuan.interactable_object_at_tile(5, 36).get("properties", {}).get("action", "")) == "delete_save", "歪脖树应由危险树原型提供删档行为")
	_assert(str(kaiyuan.interactable_object_at_tile(15, 36).get("properties", {}).get("action", "")) == "drink", "水井应由饮水点原型提供行为")
	_assert(str(kaiyuan.interactable_object_at_tile(39, 3).get("properties", {}).get("worldEventId", "")) == "kaiyuan_north_sign", "视觉路牌与事件摆放应在同一格命中")
...
func _load_map(data_registry: Node, map_id: String) -> TiledMapLoader:
	for map_path in data_registry.map_files:
		if map_path.get_file().get_basename() == map_id:
			var context := TiledMapLoader.new()
			_assert(context.load_file(map_path), "地图应成功加载：" + map_id)
