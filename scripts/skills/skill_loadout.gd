	...
	for skill_id in equipped_skill_ids():
		...
		for move in definition.get("moves", []):
			...
			if current_level >= unlock_level:
				result.append({"skill_id": str(skill_id), "name": move.get("name", "招式"), "kind": kind, "unlock": unlock_level, "level": current_level, "combat_effects": move.get("combatEffects", {})})
	return result
...
func unlocked_ults() -> Array:
	...
	if arch_level >= ABILITY_RULES.ULT_TIER2_ARCH_LEVEL:
		result.append(ABILITY_RULES.build_ult(ult, 2, inner_power, arch_level))
	return result
