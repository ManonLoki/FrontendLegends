...
func refresh() -> void:
	...
	_portrait(game.npc_texture, NpcSystem.sprite_region_for_instance(view.enemy), Vector2(right_x, 72.0), Vector2(44.0, 56.0))
	_render_stats(area)
	if not view.ended:
...
func _label(text: String, rect: Rect2, font_size: int, alignment := HORIZONTAL_ALIGNMENT_LEFT, color := Color("292b26")) -> Label:
	...
	label.add_theme_font_size_override("font_size", maxi(10, int(round(float(font_size) * game._display_scale()))))
